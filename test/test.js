...
contract("Employees", async () => {
    it("should create a first new employee", async () => {
        ...
        await instance.createEmployee("Ivo", "Norway", "10102020", worker1, "Viken Blockchain Solutions", department);
        let employee = await instance.getEmployee(0);
        assert(employee.id == 0);
    });
    it("should create a second new employee", async () => {
        let instance = await Employees.deployed();
        await instance.createEmployee("Sienna", "Norway", "04012021", worker2, "Viken Blockchain Solutions", department);
    ...
    it("shouldn't let the worker address and department address be the same address", async () => {
        let instance = await Employees.deployed();
        await truffleAssert.fails(instance.createEmployee("Samuel", "Norway", "21.12.2020", department, "Viken Blockchain Solutions", department), truffleAssert.ErrorType.REVERT);
    });
});


/* 
await truffleAssert.fails(instance.createWorker("Ivo", "NORWAY", 10102020, worker1, "Viken Blockchain Solutions", department ,{}));
